export default function EmergencyBanner({ severity = 'high', onDismiss }) {
    ...
    >
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          >
            <svg
              className={`w-4 h-4 ${isCritical ? 'text-white' : 'text-red-600'}`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
          ...
          <div>
            <p
              className={`text-sm font-bold
                          ${isCritical ? 'text-white' : 'text-red-800'}`}
            >
              {isCritical ? '🚨 Emergency Help' : 'Help is available'}
            </p>
            <p
              className={`text-[11px] ${isCritical
                ? 'text-red-200'
                : 'text-red-500'}`}
            >
              Free · 24x7 · Confidential
            </p>
          </div>
        </div>
        {onDismiss && (
        ...
        {HELPLINES.map((h, i) => (
          ...
          >
            <div
              className={`w-7 h-7 rounded-full flex items-center
                          justify-center shrink-0
                          ${isCritical ? 'bg-red-400' : 'bg-red-100'}`}
            >
              <svg
            ...
            <div className="min-w-0">
              ...
              <p
                className={`text-[10px] truncate
                            ${isCritical ? 'text-red-200' : 'text-red-500'}`}
              >
                {h.label}
              </p>
            </div>
