...
class SakhiBotState(TypedDict):
    ...
    final_plan:      list
    safety_quality:  dict | None   # ← NEW
    document_ready:  bool
    document_type:   str
    document_form:   dict | None   # ← NEW
    next_question:   str
...
def document_node(state: SakhiBotState) -> SakhiBotState:
    ...
    # doc_run() (the form-based doc_drafter) already returns document_form
    # in its result dict — it just flows through here unchanged.
...
def safety_node(state: SakhiBotState) -> SakhiBotState:
        ...
        return {
            **state,
            "safety_result": {
                ...
                "next_question":   "",
                "ready":           False,
                "quality_verdict": None,   # ← NEW, matches safety_planner's new return shape
            }
        }
    ...
    result = safety_run(
        state["message"],
        state.get("history", []),
        retrieved_sources=state.get("legal_sources", [])   # ← NEW
    )
...
def synthesizer_node(state: SakhiBotState) -> SakhiBotState:
    ...
    # When a document form is being offered, don't duplicate the "I will help
    # you prepare a..." text as a separate chat line — the form card itself
    # carries the title and instructions. Fall back to the plain message only
    # if no form was generated (e.g. an error occurred before form-building).
    if doc_result.get("needs_document") and doc_result.get("message") and not doc_result.get("document_form"):
        answer_parts.append(doc_result["message"])
    elif doc_result.get("needs_document") and doc_result.get("document_form"):
        answer_parts.append(doc_result.get("message", ""))
    elif legal_answer:
    ...
    return {
        **state,
        ...
        "safety_quality":  safety_result.get("quality_verdict"),   # ← NEW
        "document_ready":  doc_result.get("document_ready", False),
        "document_type":   doc_result.get("document_type", ""),
        "document_form":   doc_result.get("document_form"),   # ← NEW
...
def run(
    ...
    return {
        ...
        "safety_quality":   result["safety_quality"],   # ← NEW
