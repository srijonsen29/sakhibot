...
from core.consent import (
    grant_consent, revoke_consent, check_consent, get_user_consents, get_active_consents,
    check_feature_consent, get_consent_text, bulk_grant_consents, export_consent_history,
...
async def list_consent_policies():
    return {
        "policies": [{
            "consent_type": p.consent_type, "required": p.required, "purpose": p.purpose,
            "data_types": p.data_types, "legal_basis": p.legal_basis, "retention": p.retention_period
...
async def grant_user_consent(req: GrantConsentRequest):
    try:
        record = grant_consent(req.user_id, req.consent_type, req.duration_days)
        return {
            "status": "consent_granted", "consent_id": record.consent_id,
            "consent_type": record.consent_type, "purpose": record.purpose,
...
async def grant_bulk(req: BulkConsentRequest):
    ...
    return {
        "status": "consents_granted", "total_granted": len(records),
        "consents": [{"consent_type": r.consent_type, "granted_at": r.granted_at} for r in records]
    }
...
async def revoke_user_consent(req: RevokeConsentRequest):
    ...
    return {"status": "consent_revoked", "consent_type": req.consent_type}
...
async def check_feature_access(user_id: str, feature: str):
    ...
    return {
        "user_id": user_id, "feature": feature, "allowed": result["allowed"],
        "missing_consents": result["missing_consents"], "required_consents": result["required_consents"]
    }


@router.get("/list/{user_id}")
async def list_user_consents(user_id: str, active_only: bool = False):
    records = get_active_consents(user_id) if active_only else get_user_consents(user_id)
    return {
        "user_id": user_id, "total": len(records),
        "consents": [{
            "consent_id": r.consent_id, "consent_type": r.consent_type, "granted": r.granted,
            "purpose": r.purpose, "data_types": r.data_types, "granted_at": r.granted_at,
