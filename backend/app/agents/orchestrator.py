...
from app.agents.legal_retriever  import run as legal_run
from app.agents.doc_drafter      import run as doc_run, detect_document_type
from app.agents.resource_locator import run as resource_run, needs_location
from app.agents.safety_planner   import run as safety_run, extract_situation_from_history
...
def build_graph():
    ...
    graph.add_edge("router",      "document")
    graph.add_edge("router",      "resource")
    graph.add_edge("router",      "safety")

    # fan-in to synthesizer
    graph.add_edge("legal",       "synthesizer")
    graph.add_edge("document",    "synthesizer")
    graph.add_edge("resource",    "synthesizer")
