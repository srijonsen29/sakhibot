...
def audit_form_fields(doc_type: str, fields: dict, required_fields: list) -> dict:
    """
    Sanity + completeness check for a USER-SUBMITTED form.
    Unlike audit_doc_fields (which checks LLM-extracted values against
    conversation history for hallucination), values here were typed
    directly by the user in the rendered form, so there is no
    traceability risk to audit — only completeness and basic sanity
    (missing required fields, malformed phone number, an
    implausibly short incident description).
    """
    warnings = []
    missing_fields = []
    ...
    phone = str(fields.get("complainant_phone", "")).strip().replace(" ", "").replace("-", "")
    if phone and not PHONE_PATTERN.match(phone):
        warnings.append("Phone number does not look like a valid 10-digit Indian mobile number.")

    description = str(fields.get("incident_description", "")).strip()
    if description and len(description.split()) < MIN_DESCRIPTION_WORDS:
        warnings.append("Incident description looks too short to be meaningful.")

    verdict = {
        "passed": not warnings,
        "score": 100 if not warnings else max(40, 100 - 15 * len(warnings)),
        "warnings": warnings,
        "missing_fields": missing_fields,
        "doc_type": doc_type,
        "final_action": "pass" if not warnings else "allow_with_log",
    }
    log_verdict("doc_drafter_form", verdict)
    return verdict
