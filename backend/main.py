...
class DocumentFormSubmission(BaseModel):
    document_type: str
    fields:        dict
    language:      str = "en"
...
# ── document download endpoint (legacy, one-question-at-a-time flow) ─────────
...
# ── document form submission endpoint (new single-message form flow) ─────────
@app.post("/api/document/form")
async def submit_document_form(req: DocumentFormSubmission, current_user=Depends(get_current_user)):
    from agents.doc_drafter import generate_from_form, REQUIRED_FIELDS
    from quality_gates import audit_form_fields

    required = REQUIRED_FIELDS.get(req.document_type, [])
    verdict = audit_form_fields(req.document_type, req.fields, required)

    if not verdict["passed"]:
        # Don't generate from an incomplete/invalid form — send back exactly
        # which fields are missing/invalid so the frontend can highlight them
        # without the user losing anything they already typed.
        return JSONResponse(status_code=422, content={"verdict": verdict})

    try:
        pdf_bytes, _ = generate_from_form(req.document_type, req.fields, req.language)
